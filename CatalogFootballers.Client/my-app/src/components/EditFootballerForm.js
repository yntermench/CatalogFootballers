...
export default function EditFootballerForm() { 
  ...
  useEffect (() => {
    fetch(process.env.REACT_APP_API + `/footballer/get/${id}`, {
      method: "GET",
      headers: {
  ...
  const handleSubmit = (e) => {
    ...
    fetch(process.env.REACT_APP_API + `/footballer/edit/${id}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  return (
    <div>
      <form className="w-200 px-5">
        <h1 className="mt-3">Изменить футболиста</h1>
        ...
        <div className="mt-5">
          ...
          >
            ...
            {titlesCommands.map((titleCommand) => (
              <option key={titleCommand.id} value={formData.title} data-id={titleCommand.id}>
                {titleCommand.title}
              </option>
            ))}
